	...
	axisShader(std::string(PROJECT_DIRECTORY) + "/shaders/axis/vertex.glsl", std::string(PROJECT_DIRECTORY) + "/shaders/axis/fragment.glsl")
{
	...
	gridShader.use();
	gridShader.set("model", glm::mat4(1.0f));

	axisShader.use();
	axisShader.set("model", glm::mat4(1.0f));
}
...
{	
	...
	glBindVertexArray(vaoG);
	gridShader.use();
	gridShader.set("view", view);
	gridShader.set("proj", projection);
	...
	axisShader.set("proj", projection);

	axisShader.set("color", glm::vec3(1.0f, 0.0f, 0.0f));
	glDrawArrays(GL_LINE_STRIP, 0, 2);
	axisShader.set("color", glm::vec3(0.0f, 1.0f, 0.0f));
	glDrawArrays(GL_LINE_STRIP, 2, 2);
	axisShader.set("color", glm::vec3(0.0f, 0.0f, 1.0f));
